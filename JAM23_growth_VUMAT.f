...
! Fatemeh Jafarabadi, March 2022, Modified for Pressure Model
...
c          do km=1,nblock
c             ! store all the CURRENT coordinates 
c             ! at the reduced integration point location 
c             StorageOld(nElement(km)-BaseElement,1) = coordMp(km,1)
c             StorageOld(nElement(km)-BaseElement,2) = coordMp(km,2)
c             StorageOld(nElement(km)-BaseElement,3) = coordMp(km,3)
c          enddo
     ...
     +                     coordx,coordy,coordz,curvature,NormCurve,a0)
     ...
     +                     coordx,coordy,coordz,curvature,NormCurve,a0)
         ...
         stateNew(km,7) = a0(1,1)
         stateNew(km,8) = a0(2,1)
         stateNew(km,9) = a0(3,1)
...
****************************************************************************
     ...
     +                      curvature,NormCurve,a0)
      ...
      real*8 Fginv(3,3)
      real*8 angle_alpha,angle_beta,angle_gamma,Rmat(3,3)
      real*8 vec_unrot(3,1),vec(3,1)
      real*8 a_const,b_const
     ...
     +     third=1.d0/3.d0,nine=9.d0,Pi=3.1415926d0)
      ...
      ! 3d ellipsoid
      ! here I rotate back to the unrotated state 
      ! 
c      angle_alpha = 0.0 
c      angle_beta  = pi/2.0  
c      angle_gamma = 0.0 
c      call rotate(angle_alpha,angle_beta,angle_gamma,Rmat)
c
c      vec(1,1) = coordx
c      vec(2,1) = coordy
c      vec(3,1) = coordz
c
c      vec_unrot = matmul(Rmat,vec)
c     
c      ! length of principal axis 
c      a_const = 0
      b_const = 9/7
c   
c      !  get the outnormal at unroted state 
c      a0(1,1) = 2.0*coordx*b_const
c      a0(2,1) = 2.0*coordy
c      a0(3,1) = 2.0*coordz*b_const
c
c      ! rotate to the current 
c      angle_alpha = 0.0 
c      angle_beta  = -pi/2.0 
c      angle_gamma = 0.0 
c      call rotate(angle_alpha,angle_beta,angle_gamma,Rmat)
c      a0 = matmul(Rmat,a0)
c      a0 = matmul(F_tau,a0)    
...
****************************************************************************
      subroutine rotate(angle_alpha,angle_beta,angle_gamma,Rmat)

      ! This subroutine will rotate the position vector 
  
      implicit none

      real*8 Rmat(3,3),Rx(3,3),Ry(3,3),Rz(3,3)
      real*8 angle_alpha,angle_beta,angle_gamma

      Rz(1,1) = cos(angle_gamma)
      Rz(1,2) = -sin(angle_gamma)
      Rz(1,3) = 0.0   
      Rz(2,1) = sin(angle_gamma)
      Rz(2,2) = cos(angle_gamma)
      Rz(2,3) = 0.0
      Rz(3,1) = 0.0
      Rz(3,2) = 0.0
      Rz(3,3) = 1.0

      Ry(1,1) = cos(angle_beta)
      Ry(1,2) = 0.0 
      Ry(1,3) = sin(angle_beta)
      Ry(2,1) = 0.0
      Ry(2,2) = 1.0 
      Ry(2,3) = 0.0
      Ry(3,1) = -sin(angle_beta) 
      Ry(3,2) = 0.0
      Ry(3,3) = cos(angle_beta)

      Rx(1,1) = 1.0
      Rx(1,2) = 0.0
      Rx(1,3) = 0.0 
      Rx(2,1) = 0.0
      Rx(2,2) = cos(angle_alpha)
      Rx(2,3) = -sin(angle_alpha)
      Rx(3,1) = 0.0
      Rx(3,2) = sin(angle_alpha)
      Rx(3,3) = cos(angle_alpha)
      
      ! total rotation matrix
      Rmat = matmul(Rz,matmul(Ry,Rx))
