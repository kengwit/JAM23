...
def Create_Material(ModelName, Materials, ratio): 
    ...
    mdb.models[ModelName].materials['WHIT'].Hyperelastic(materialType=ISOTROPIC, testData=OFF, type=NEO_HOOKE, volumetricResponse=VOLUMETRIC_DATA, table=((mu_subcortex/2, 2/bulk_subcortex), ))

    mdb.models[ModelName].Material(name='GRAY')
    mdb.models[ModelName].materials['GRAY'].Density(table=((density, ), ))
    mdb.models[ModelName].materials['GRAY'].Hyperelastic(materialType=ISOTROPIC, testData=OFF, type=NEO_HOOKE, volumetricResponse=VOLUMETRIC_DATA, table=(((ratio*mu_subcortex)/2, 2/(ratio*bulk_subcortex)), ))
#######################################################################################
#######################################################################################
...
#######################################################################################
#######################################################################################
def Create_Boundary_Conditions(ModelName, InstanceName, Step, displacement):
    a = mdb.models[ModelName].rootAssembly   
    ...
    mdb.models[ModelName].DisplacementBC(name='bottom', createStepName='pressure', region=region, u1=UNSET, u2=0.0, u3=0.0, ur1=UNSET, ur2=UNSET, ur3=UNSET, amplitude='Amp-1', fixed=OFF, distributionType=UNIFORM, fieldName='', localCsys=None)
    ...
    mdb.models[ModelName].DisplacementBC(name='side-2', createStepName='pressure', region=region, u1=-displacement, u2=UNSET, u3=UNSET, ur1=UNSET, ur2=UNSET, ur3=UNSET, amplitude='Amp-1', fixed=OFF, distributionType=UNIFORM, fieldName='', localCsys=None)
    ...
    mdb.models[ModelName].DisplacementBC(name='ref-right', createStepName='pressure', region=region, u1=-displacement, u2=0.0, u3=0.0, ur1=0.0, ur2=0.0, ur3=0.0, amplitude='Amp-1', fixed=OFF, distributionType=UNIFORM, fieldName='', localCsys=None)
...
def Create_Output(ModelName, PartName, InstanceName, Step): 
    ...
    mdb.models[ModelName].FieldOutputRequest(name='F-Output-2', createStepName=StepName, variables=('S', 'PE', 'LE', 'U', 'ELSE', 'COORD'),  numIntervals=500, region=regionDef, sectionPoints=DEFAULT, rebar=EXCLUDE)

    regionDef=mdb.models[ModelName].rootAssembly.allInstances[InstanceName].sets['Cortex']
    mdb.models[ModelName].FieldOutputRequest(name='F-Output-3', createStepName=StepName, variables=('ELSE', 'COORD', 'U'), numIntervals=500, region=regionDef, sectionPoints=DEFAULT, rebar=EXCLUDE)

    regionDef=mdb.models[ModelName].rootAssembly.allInstances[InstanceName].sets['Subcortex']
    mdb.models[ModelName].FieldOutputRequest(name='F-Output-4', createStepName=StepName, variables=('ELSE', 'COORD', 'U'), numIntervals=500, region=regionDef, sectionPoints=DEFAULT, rebar=EXCLUDE)
...
for j in range(5):
    for i in range(31):
            ...
            ewidth = 300                                                    # number of elements in length
